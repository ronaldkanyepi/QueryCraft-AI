...
async def main():
    async with init_memory() as memory:
        ...
        #     HumanMessage(
        #         content="Hi, my name is Ronald Kanyepi, and I work in the marketing department of Partners Bank in Maine."),
        #     AIMessage(
        #         content="Nice to meet you, Ronald Kanyepi! I'll remember that you work in marketing at Partners Bank in Maine. How can I help you today?"),
        ...
        profile_results = await memory["store"].asearch(
            ("users", "334438404911529987", "profile"),
            limit=10,
        )
        ...
        general_results = await memory["store"].asearch(
            ("memories", "334438404911529987", "episodic"),
            limit=10,
        )
        ...
        # await create_manage_memory_tool(namespace=("users", "778", "profile"), store=memory["store"]).ainvoke({"content":"My name is Baba Blessed"},schema=UserProfile,actions_permitted=['update','insert'],config={"configurable": {"thread_id": "8000", "user_id": "778"}})
